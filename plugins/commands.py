...
from info import CHANNELS, ADMINS, AUTH_CHANNEL, LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, M_GRP_LINK, PROTECT_CONTENT, CHNL_LNK, GRP_LNK, REQST_CHANNEL, SUPPORT_CHAT_ID, MAX_B_TN, VERIFY
...
async def save_template(client, message):
    ...
    await sts.edit(f"Sᴜᴄᴄᴇssғᴜʟʟʏ ᴄʜᴀɴɢᴇᴅ ᴛᴇᴍᴘʟᴀᴛᴇ ғᴏʀ {title} ᴛᴏ:\n\n{template}")
                ...
                #==================Restart Function==================#
@Client.on_message(filters.private & filters.command(["restart", "r"]) & filters.user(ADMINS) & filters.group) 
...
async def deletemultiplefiles(bot, message):
    ...
    deleted = 0
    for file in files:
        await k.edit_text(f"<b>Process started for deleting files from DB. Successfully deleted <code>{str(deleted)}</code> files from DB for your query <code>{keyword}</code> !\n\nPlease wait...</b>")
