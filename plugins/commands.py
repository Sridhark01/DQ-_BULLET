...
async def deletemultiplefiles(bot, message):
    ...
    deleted = 0
    for file in files:
        await message.reply(f"<b>Process started for deleting files from DB. Successfully deleted <code>/{total}</code> files from DB for your query <code>{keyword}</code> !\n\nPlease wait...</b>") # {str(deleted)}
